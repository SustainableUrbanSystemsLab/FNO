"""
PINN-FNO Diagnostic Tool
=========================
Generates a diagnostic plot comparing PINN-FNO predictions vs ground truth.
Identical to diagnose_hybrid.py but uses the PINNFNO model class.

Usage:
    uv run python tools/diagnose_pinn.py --model pinn_fno_weights.pth --data test_csv/your_file.csv
    uv run python tools/diagnose_pinn.py --model pinn_fno_weights.pth --data test_csv/your_file.csv --output pinn_diagnostic.png
"""
...
def run_diagnostics(model_path, data_path, output_name="pinn_diagnostic.png"):
    print(f"--- PINN-FNO Diagnostic: {os.path.basename(model_path)} ---")
    ...
    # 2. Load Checkpoint first to handle architecture discovery
    ...
    # Architecture discovery from Config/Weights
    config  = load_config()
    ...
    try:
        ...
        print(f"Saved weights don't match current inferred: (modes={MODES1}, width={WIDTH}, layers={N_LAYERS}).")
        print(f"\nFull error: {e}")
        return
    model.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="pinn_fno_weights.pth")
    parser.add_argument("--data",  type=str, required=True,
                        help="Path to a test CSV with ground truth mag_U column")
    parser.add_argument("--output", type=str, default="pinn_diagnostic.png")
    args = parser.parse_args()

    if not os.path.exists(args.model):
        # Try root-level fallback
        alt = args.model.replace("weights/", "")
